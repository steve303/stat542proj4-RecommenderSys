...
Note, if a movie is not rated by a user, having a value NA, and is in the neighborhood of the active user, then that rating is ignored.    
...
In an attempt to improve our algorithm, we performed an optimization to specify k.  Slight improvement could be made by increasing k = 75, but the improvement was less than 1% compared to k = 25. (figure 2).



```{r optimize01, cache=TRUE, results='hide'}

algorithms_nn01 <- list("UBCF_50" = list(name = "UBCF", param = list(normalize = "Z-score", method = "Cosine", nn = 50)),
                   "UBCF_75" = list(name = "UBCF", param = list(normalize = "Z-score", method = "Cosine", nn = 75)),
                   "UBCF_100" = list(name = "UBCF", param = list(normalize = "Z-score", method = "Cosine", nn = 100)))
...
table_n50 = data.frame("RMSE" = rep(0, 10), "MSE" = rep(0,10), "MAE"= rep(0,10))
table_n75 = data.frame("RMSE" = rep(0, 10), "MSE" = rep(0,10), "MAE"= rep(0,10))
table_n100 = data.frame("RMSE" = rep(0, 10), "MSE" = rep(0,10), "MAE"= rep(0,10))

for (i in 1:10){
  table_n50[i,] = eval_ubcf_nn01[[1]]@results[[i]]@cm
  table_n75[i,] = eval_ubcf_nn01[[2]]@results[[i]]@cm
  table_n100[i,] = eval_ubcf_nn01[[3]]@results[[i]]@cm
}

```

<br>
*Figure 2. Average UBCF RMSE Values for k = 25, 50, 75, 100*
```{r, eval=TRUE, echo=FALSE}

barplot( height = c(mean(table_ubcf$RMSE), mean(table_n50$RMSE),mean(table_n75$RMSE), mean(table_n100$RMSE[-8])), 
         names.arg = c("k=25", "k=50","k=75", "k=100"), main = "10 Fold Average RMSE - K Optimization", ylab = "error")
```
...
## Summary
In the end our app was successfully deployed without much delay predicting recommendations.  There are several areas which could be improved for system two.  These include, providing more movie choices to rate, allowing the user to apply a filter, i.e. year, director.  Since our chosen UBCF algorithm did not perform significantly better than the random selection algorithm, this suggests there is room for improvement.  One idea is to use user profile data such as age, sex, profession to create more representative similarity scores between users.
